...
You will often need to import data into MATLAB, and that data will likely be stored in a file somewhere on your computer. The good news is that MATLAB makes it easy to find, access, and work with these files.
...
When you launch MATLAB, it automatically opens a folder on your hard drive. This is known as the "current folder," and its contents are shown in a window in MATLAB called the "Files" window.

![Files Window](images/Files-Window.png){ width="650"}
...
### Now You try: Inspect a Folder on your Computer

1. Click on the Browse for Folder icon ![open folder icon][L-img-browse-dir]
2. Select your Documents folder
3. Review the address bar. You should see something like this: ![matlab address bar](images/address-bar-documents-folder.png){width=250px}
4. Review the contents of the "Files" window. You should see documents that look specific to your computer.
...
!!! note "NOTE: Folder vs File paths"
    ...
    A path to a folder and a path to a file can look nearly identical. Both typically contain a series of folder names separated by file separators. To tell them apart, look at the last name in the path. If it includes a file extension, such as `.txt`, `.csv`, or `.mat`, the path likely points to a file. If there is no extension, the path likely points to a folder.

### The File Separator

The file separator is the character used to separate folder and file names in a path string. The separator differs between macOS and Windows.

In MATLAB, you can use the **`filesep`** function to return the correct file separator for your operating system:
...
### Mac vs Windows File separators

On macOS (and Linux), folders and files are separated by a forward slash (`/`). On Windows, they are separated by a backward slash (`\`) and drives are indicated by a colon (`:`) or a double backward slash (`\\`).
...
In the representation above, the top name is the top-level folder, and each folder below it is contained within the folder above it. For example, the `MATLAB` folder is inside the `Documents` folder, which is inside the `ernesto` folder, which is inside the `Users` folder.

On macOS, you can represent this folder hierarchy as a one-line character array, with folder names separated by forward slashes (/):
...
On Windows, you use the back slash (`\`) as a file separator, so a similar folder hierarchy on a Windows PC might be saved as the following path string:

```title="Windows path"
'C:\Users\Ernesto\Documents\MATLAB'
```

…This path points to the MATLAB folder in the Documents folder in the `Ernesto` folder in the Users folder on the C Drive.
...
!!! note "Absolute vs Relative Paths"
    ...
    - A **relative** path is *relative* to the "Current Folder" in MATLAB. For example, in my MATLAB folder, there is a folder called "Wild Mice". A relative path to this folder would simply be `'Wild Mice'`. In this case, MATLAB assumes the top folder to be the MATLAB folder. Inside the "Wild Mice" folder is a folder called "wmdb". The relative path to this folder is `'Wild Mice/wmdb'`.
...
This will convert the display to a string that is already conveniently highlighted and can be copied using standard copy operations (e.g. Ctrl-C or Cmd-C).

[L-img-path-string]: images/address-bar-path-string.png

### Try It Now: Assign current folder path to a variable

Assign the Current Folder path to a new variable *mycurrentfolder* using the following steps:
...
Both Mac and Windows allow you to copy file paths using their system file browsers. You then simply paste the path as a character array in MATLAB and then assign to a variable, as we did above.
...
When creating file paths in MATLAB, don't forget that paths need to be character arrays, so they need to be bracketed by single quotes:
...
1. **`uigetdir`**: For selecting folders
2. **`uigetfile`**: For selecting files

Both functions work fairly similarly in that they bring up the system file browser dialog, which allows the user to select a file or a folder. Once the User selects "OK", the function saves the name of the file or the folder and the path to separate variables.
...
If we want to generate a path to a file using the file browser, then we use the function **`uigetfile`**.
...
…Notice that the function actually returns the path as two parts: the filename (including its extension) and the folder path.
...
**Note**: If you click Cancel, the output variables (*`Filename`* and *`Pathname`*) are set to ZERO. This is useful for control flow statements, as zeros resolve to a `FALSE`. For example, you could follow a **uigetfile** call with an IF ELSE statement, as follows:

```matlab linenums="1" title="Using uigetfile outputs in a control flow"
...
After we use **`uigetfile`**, we get the file path in two parts: the filename and the path to the file. How do we merge the data into a full path? One way might be to concatenate the different strings. But you will have to make sure to use the right file separators at the right locations.

An easier way to merge two pieces of information into a file path is to use the function **`fullfile`**. For example, we can take the outputs from **`uigetfile`**, *`Filename`* and *`Pathname`*, and stitch them into a proper full path, as follows
...
…The variable *`path`* contains the full path string to the folder while *`folder_name`* contains the name of the current folder.
...
…In this code, we use the output of **`pwd`** as the input, which returns the folder path to the current folder. In this example, this path is '/Users/ernesto/Documents/MATLAB'. So, **`fileparts`** returns the path to the folder containing the MATLAB folder (`'folder_path'`) and the name of the current folder (`'MATLAB'`).
...
The function **`cd`** programmatically changes the current directory. The input is a folder path. In this example, we will use *`weather_folder`*, which we previously set to be '/Users/ernesto/Documents/Unit 1/weather_data'.
...
contents = 
  ...
  struct with fields:
       ...
       name: 'w2013.csv'
     folder: '/Users/ernesto/Documents/Unit 1/weather_data'
...
The structure array *`contents`* has 6 fields:

- **name** - the name of the file

- **folder** - the full path to the folder containing the file

- **date** - the modification date of the file

- **bytes** - the size of the file

- **isdir** - logical value that is TRUE if the item is a folder
...
…Notice that we get a 14×1 structure array. This means that the function **`dir`** found 14 items in the 'weather_data' folder. However, not everything it finds is an actual file.

#### Extracting data about a file or folder from the *`contents`* structure array

Since the variable *`contents`* is a structure array, we can index it like we would any structure array.
...
…The result is a 1×1 structure array with details about the fifth file.
...
…The problem with this approach is that you just get a comma-separated list, with the individual names spit out one after the other into the command window.
...
If you review all the names in the "name" field of `contents`, you'll notice that only the last 10 elements refer to an actual file (a .csv file). The rest of the elements are names that refer to hidden files (files whose file names start with a period) and two navigation names: '.' and '..', which refer to the current directory and the directory above it, respectively (1).
{ .annotate}

1. Don't worry too much about the navigation names. They are confusing to use.
...
What you really want **`dir`** to search for is any file name that *ends* with a '.csv'. To do this, you use the asterisk in the file name. The function **`dir`** interprets the asterisk in a file name as a wildcard character. Whenever you include an asterisk, that means 'any character or characters'.

For example, to find all the files in the current folder that end with a `.csv`, use the following syntax.

```matlab linenums="1" title="Get info on files with the .csv extension "
contents = dir('*.csv')
```
...
Now we get a 10×1 structure array. This means that there are 10 files with the `.csv` extension in the "Current Folder". The function **`dir`** ignores all of the hidden files and directories.

---

## Loading Files

Up to this point, we have only been creating file paths, which are just character arrays that contain the location, or address, of a file on your computer. We have not actually loaded any of the data from that file into MATLAB.
...
2. We use **`numel`** to count the number of elements in the structure array *`contents`*, which is equal to the number of .csv files in *`weather_folder`*.
3. In the *initializing statement* of the FOR LOOP, we create an array *`n`* that has the same number of elements as there are files in the folder
4. On each iteration of the loop, we:
...
And we're done: a quick and easy way to read in all of the files from a selected folder. Note that this only works because the data in the spreadsheet files all have the same number of columns with the same column headers.
