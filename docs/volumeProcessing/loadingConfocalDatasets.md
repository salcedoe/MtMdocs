...
If you haven't installed the `bfmatlab` tools or if you have a basic TIFF dataset, you can use **`imread`** to load the volume.
...
…Here we review the first element in the metadata structure and we find that each slice in the image stack is an unsigned 8-bit image with 512 rows (Height) and 512 columns (Width). So, the size of our image stack is `512x512x12`.
